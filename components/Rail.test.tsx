...
// Rail이 품은 ThemeToggle이 스토어를 구독하므로 Supabase 경계만 최소로 대체한다
// (ThemeToggle.test.tsx와 동일 패턴).
...
function renderRail(overrides: Partial<RailProps> = {}) {
  ...
  const utils = renderWithStore(<Rail {...props} />)
  return { ...utils, props }
}
...
describe('접힘 상태 렌더링 (C-4 ~ C-9)', () => {
  ...
  test('접힘 상태에서 내비 버튼이 라벨 이름을 갖고 onNav를 호출한다 (C-5 · C-6)', async () => {
    const user = userEvent.setup()
    const { props } = renderRail({ collapsed: true })

    const keys = Object.keys(NAV_LABELS) as NavKey[]
    for (const key of keys) {
      await user.click(screen.getByRole('button', { name: NAV_LABELS[key] }))
      expect(props.onNav).toHaveBeenLastCalledWith(key)
    }
    expect(props.onNav).toHaveBeenCalledTimes(keys.length)
  })

  test('접힘 상태에서도 활성 내비 항목이 구분된다 (C-5, FR-013)', () => {
    renderRail({ collapsed: true, nav: 'all' })

    expect(screen.getByRole('button', { name: '전체 페이지' })).toHaveClass('active')
  })

  test('내비 라벨은 span.navitem-label로, 카운트는 .count로 래핑된다 (C-6)', () => {
    const { container } = renderRail()

    expect(container.querySelectorAll('.navitem .navitem-label')).toHaveLength(1)
    // counts prop이 all 한 항목만 제공하므로 .count도 1개
    expect(container.querySelectorAll('.navitem .count')).toHaveLength(1)
  })
  ...
  test('접힘 검색 버튼 클릭 시 레일이 펼쳐지고 검색 입력에 포커스가 놓인다 (C-7, FR-015)', async () => {
    const user = userEvent.setup()
    renderWithStore(<CollapsibleHarness initialCollapsed />)
...
describe('접근성·툴팁 (US3: C-2 · C-8)', () => {
  ...
  test('접힘 내비의 data-tip은 카운트가 있으면 포함하고 없으면 라벨만 쓴다 (C-8)', () => {
    renderRail({ collapsed: true })
    expect(screen.getByRole('button', { name: '전체 페이지' })).toHaveAttribute(
      'data-tip',
      '전체 페이지 (3)',
    )
  })

  test('카운트가 없으면 data-tip은 라벨만 쓴다 (C-8)', () => {
    renderRail({ collapsed: true, counts: {} })
    expect(screen.getByRole('button', { name: '전체 페이지' })).toHaveAttribute(
      'data-tip',
      '전체 페이지',
    )
